...
현재 repository는 Owner Review 및 Local Policy / Approval Request Baseline 단계다. 단순히 Review 상태만 저장하는 것이 아니라 `policy_decisions`에 기반하여 위험도(Risk Level)를 평가하고, Squash Merge(`merge.perform_squash`) 수행 시 해시 기반 Fingerprint로 묶인 명시적인 `approval_requests` 승인을 검증한다. Fingerprint는 base/result commit뿐만 아니라 정규화된 실행 인자(Normalized execution arguments hash, 예: `commit_message`)를 포함하여, Base branch, Result, 또는 실행 인자가 변하면 Approval은 Stale 처리되어 안전을 보장한다. (보안상 원시 인자 값이나 Secret은 Fingerprint에 평문 저장하지 않는다)
...
이 Slice의 Git 동작은 status/read-only 확인에 한정한다. Dirty repository도 등록과 읽기·분석은 허용하지만 후속 Worker 실행은 dirty 상태에서 차단할 예정이다. Conversation/Tool/Task/Worker, worktree/branch/commit/merge가 구현되었고, 외부 프로세스 실행을 캡슐화한 Process Runner Baseline(환경변수 allowlist 적용, 민감정보 차단, **Background Execution을 위한 STDIN DEVNULL 강제 연결**)과 안전한 External CLI Adapter Contract (Dry Run), 그리고 **External CLI Active 일방 Run Guard(동일 TaskAttempt 내 중복 실행 차단)**가 추가되었다. Process Runner는 백그라운드 구동에 특화되어 STDIN 입력을 닫아 무한 대기(Block)를 방지하며, 대화형(Interactive) 실행은 지원하지 않는다. 실제 Codex/Antigravity CLI 연동, credential injection, Remote Test Runner와 Desktop packaging은 아직 구현하지 않는다. (로컬 시스템에 `agy` 실행 파일이 있음을 확인하여, 격리된 worktree 환경에서 README.md 한 줄 추가만 허용하는 "Controlled AGY CLI Worktree Test"를 실험적으로 적용하였다. API는 controlled prompt 모드만 허용하며, 실제 agy execution은 로컬 환경 설정으로 opt-in한 경우에만 수동 검증할 수 있다.)
...
실제 AGY의 controlled README 결과가 Owner review, 명시적 승인, squash 준비와 squash merge로 이어지는 경로는 tmp repository 전용 opt-in E2E로 검증한다. 기본 pytest에서는 skip하며 implementation repository를 AGY target으로 사용하지 않는다.
