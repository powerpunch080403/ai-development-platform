...
- **Experimental Antigravity CLI Worker Adapter**: Antigravity CLI(`agy`)를 제한된 환경(Worktree 격리, 고정된 실행 경로, 서버 설정 제어)에서 실행하기 위한 실험적 Adapter. 기본적으로 비활성화되어 있으며, 로컬 설정으로만 경로를 지정할 수 있다. (Note: 로컬 환경 탐색 결과 `agy` 실행 파일을 발견했으며, `--print`와 `--dangerously-skip-permissions` 등 non-interactive 가능성을 확인했다. 실제 agy execution은 명시적 opt-in 상태로만 가능하며, `--dangerously-skip-permissions`는 기본으로 전달되지 않는다. 외부 API로는 controlled prompt 모드만 허용하며 free-form prompt 입력은 차단된다.)
