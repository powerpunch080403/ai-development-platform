...
- **Tool Registry**: 서버가 통제할 기능의 version, risk, idempotency, audit 요구 사항을 설명한다.
- **Tool Call Envelope**: 현재 구현 단계에서 Tool Call은 실제 실행기가 아니라 추적용 Envelope 역할만 한다. Message, Run/Step, Tool Call 기록만 저장하며, 모델/도구 부작용과 장기 Memory 기록은 포함하지 않는다.
