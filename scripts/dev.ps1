...
Write-Host "  uv run --project apps/server alembic upgrade head"
Write-Host "  uv run --project apps/server python -m aidp_server.cli auth pairing-code"
...
Write-Host "  uv run --project apps/server pytest tests/test_golden_path_readme_e2e.py"
