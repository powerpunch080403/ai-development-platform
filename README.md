...
Merge는 별도 approve API 기록이 있어야 하며 source repository가 clean이고 현재 branch/HEAD가 고정된 base와 같아야 한다. `git merge --squash`와 단일 commit만 사용하고 remote push는 하지 않는다. Team approval group, merge queue와 자동 policy는 후속이다.

Merge 성공 후 Worktree는 `cleanup_pending`이 된다. Safe cleanup은 app-managed Worktree만 `git worktree remove`로 제거하며 source repository, branch commit, artifacts와 SQLite 기록은 삭제하지 않는다. Force cleanup은 지원하지 않는다.
