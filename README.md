...
현재 저장소는 **Local User Bootstrap + Web UI Pairing Session 단계**다. SQLite migration baseline 위에 local owner bootstrap, 일회용 pairing code, HttpOnly cookie session, device/session 조회·폐기를 구현했다. Local Worker, CLI Adapter와 Remote Test Runner는 아직 구현하지 않았다.
