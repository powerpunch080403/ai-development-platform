...
현재 저장소는 **Work Item / Task / Task Attempt + Worker Claim Lease baseline 단계**다. 사람이 보는 Work Item 트리, 실행 단위 Task, 재시도별 Attempt와 Worker registry/heartbeat/5분 lease 기록을 구현했다. 실제 Worker process와 Git 작업은 아직 구현하지 않았다.
