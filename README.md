...
현재 저장소는 **Conversation / Agent Run / Tool Call 기록 baseline 단계**다. Project/Repository와 로컬 인증 위에 Conversation·Message, 실행되지 않는 Agent Run 기록, Tool Registry/Envelope와 Audit Event를 구현했다. 실제 Owner LLM, Worker, CLI Adapter와 Remote Test Runner는 아직 구현하지 않았다.
