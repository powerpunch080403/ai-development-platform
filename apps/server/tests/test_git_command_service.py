...
from aidp_server.git_commands import SubprocessGitCommandService, GitCommandError, GitCommandTimeoutError
...
def test_status_porcelain_called_process_error(service):
    with patch("subprocess.run", side_effect=subprocess.CalledProcessError(returncode=128, cmd=["git"], stderr="fatal: not a git repository")):
        with pytest.raises(GitCommandError, match="Git command failed with exit code 128: fatal: not a git repository"):
...
def test_status_porcelain_other_error(service):
    with patch("subprocess.run", side_effect=OSError("No such file or directory")):
        with pytest.raises(GitCommandError, match="Failed to execute git command: No such file or directory"):
            service.status_porcelain("/fake/path")
