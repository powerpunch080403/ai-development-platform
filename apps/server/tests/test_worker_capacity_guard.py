...
def _agent_run(app_harness: AppHarness, project_id: str) -> AgentRun:
    with app_harness.session_factory() as session:
        ...
        run = AgentRun(
            ...
            purpose="worker capacity guard test",
        )
        session.add(run)
        session.commit()
        run_id = run.id
...
def test_worker_run_stays_queued_when_worker_capacity_is_full(app_harness: AppHarness) -> None:
    ...
    queued = app_harness.client.post(
        f"/agent-runs/{run.id}/tool-calls",
        json={
            "provider_kind": "codex_cli",
            "tool_name": "worker.run_task_attempt",
            "arguments_json": {"worker_run_id": second["worker_run_id"]},
        },
    )
