...
def test_external_cli_request_schema_is_strictly_controlled():
    ...
    for mode in [
        ...
        "controlled_scope_violation_test",
        "controlled_timeout_test",
    ]:
        req = ExternalCliRunExperimentalRequest(worker_id="test_worker_id", mode=mode)
        assert req.mode == mode

    # 2. Check unsupported mode fails
    with pytest.raises(ValidationError):
        ExternalCliRunExperimentalRequest(worker_id="test_worker_id", mode="unsupported_mode_xyz")

    # 3. Check extra forbid (free-form prompt or arbitrary command fields are rejected)
    with pytest.raises(ValidationError):
        ExternalCliRunExperimentalRequest(
            worker_id="test_worker_id", mode="controlled_readme_test", prompt="Do whatever you want"
        )
