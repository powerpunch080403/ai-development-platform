...
def test_manual_worker_start_fails_if_unclaimed(app_harness: AppHarness, tmp_path: Path) -> None:
    ...
    response = app_harness.client.post(f"/task-attempts/{attempt['id']}/manual-worker/start", json={})
...
def test_manual_worker_start_fails_if_wrong_kind(app_harness: AppHarness, tmp_path: Path) -> None:
    ...
    app_harness.client.post(f"/workers/{worker['id']}/claim", json={"task_attempt_id": attempt["id"]})
...
def test_manual_worker_e2e(app_harness: AppHarness, tmp_path: Path) -> None:
    ...
    # Make manual changes
    (worktree_path / "README.md").write_text("# Manual Worker Test\nAdded manually.\n", encoding="utf-8")
    
    # Submit changes
    submit_resp = app_harness.client.post(
        f"/task-attempts/{attempt['id']}/manual-worker/submit",
        json={"commit_message": "docs: manual edit", "result_summary": "I edited the file manually"}
    )
