...
def run_mock_worker(
    session: Session, attempt: TaskAttempt, commit_message: str | None = None
) -> tuple[WorkerRun, ArtifactRef | None]:
    ...
    try:
        ...
        instruction_line = next(
            line for line in instructions.splitlines() if line.startswith(prefix)
        )
        ...
        validate_changed_paths(
            [
                ChangedPath(
                    path=rel_path,
                    status="??" if not resolved_target.exists() else " M",
                    is_new_file=not resolved_target.exists(),
                )
            ],
