...
def seed_tool_registry(session: Session) -> None:
    existing = session.execute(select(ToolRegistryEntry)).scalars().all()
    ...
    for action_def in ACTION_CATALOG:
        ...
        has_side_effect = not any(
            word in tool_name for word in ["get", "list", "check", "preview", "evaluate"]
        )
        ...
        if entry:
            ...
            session.add(
                ToolRegistryEntry(
                    ...
                    approval_behavior="approval_required"
