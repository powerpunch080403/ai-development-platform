...
def apply_authority_envelope(session: Session, tool_call: ToolCall, mode: str = "personal") -> bool:
    ...
    For this slice, read-only tools are allowed. Side-effect tools are rejected or wait for approval.
    """
    ...
    # Allow read-only tools for this slice
    if tool_call.tool_name in ["project.list", "task.list"]:
        return True
    
    # Reject everything else for now
    tool_call.status = ToolCallStatus.REJECTED
    tool_call.error_code = "NOT_IMPLEMENTED_IN_SLICE"
    tool_call.error_message = "Only read-only tools are allowed in this MVP slice"
...
def execute_read_only_tool(session: Session, tool_call: ToolCall) -> dict[str, Any]:
...
def request_owner_tool_call(
    ...
    # 4. completed / failed
    try:
        result = execute_read_only_tool(session, call)
