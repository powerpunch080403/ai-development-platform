...
def _get_attempt(session: Session, tool_call: ToolCall) -> TaskAttempt | None:
...
def _get_task(session: Session, tool_call: ToolCall, attempt: TaskAttempt) -> Task | None:
...
def accept_attempt(session: Session, tool_call: ToolCall) -> dict[str, Any]:
    attempt = _get_attempt(session, tool_call)
    if attempt is None:
        return {"error": tool_call.error_code}
    task = _get_task(session, tool_call, attempt)
    if task is None:
        return {"error": tool_call.error_code}
...
def follow_up_attempt(session: Session, tool_call: ToolCall) -> dict[str, Any]:
    source_attempt = _get_attempt(session, tool_call)
    if source_attempt is None:
        return {"error": tool_call.error_code}
    task = _get_task(session, tool_call, source_attempt)
    if task is None:
        return {"error": tool_call.error_code}
    ...
    if source_attempt.status in REVIEWABLE_STATUSES:
        if not _transition(tool_call, source_attempt, TaskAttemptStatus.REJECTED):
            return {"error": tool_call.error_code}
        source_attempt.completed_at = datetime.now(timezone.utc)
        source_attempt.result_summary = feedback
    ...
    message = _message(
        ...
        message_type=WorkRoomMessageType.OWNER_FEEDBACK,
        content=feedback,
        metadata={"action": "follow_up", "source_attempt_id": source_attempt.id},
    )
    return {
        "task_id": task.id,
        "source_task_attempt_id": source_attempt.id,
