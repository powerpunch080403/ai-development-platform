...
type SidebarSortMode = "project" | "recent" | "time";

type DiffLine = { kind: DiffLineKind; oldLine: number | null; newLine: number | null; text: string };
type DiffFile = { id: string; path: string; additions: number; deletions: number; lines: DiffLine[] };
...
function shortId(id?: string | null) { return id ? id.slice(0, 8) : "none"; }
function formatDate(value?: string | null) { return value ? new Date(value).toLocaleString() : "unknown"; }
function timeValue(value?: string | null) { return value ? new Date(value).getTime() : 0; }
...
function permissionModeLabel(value?: string | null) {
  switch (value) {
    case "ask_for_approval": return "승인 요청";
    case "approve_on_my_behalf": return "대신 승인";
    case "full_access": return "전체 허용";
    case "custom": return "맞춤형";
    default: return value ?? "권한";
  }
}
...
export function PersonalModeDashboard() {
  ...
  const [sidebarSortMode, setSidebarSortMode] = useState<SidebarSortMode>("project");
  ...
  function showActionMessage(message: string) { setActionError(message); }

  useEffect(() => {
    if (!actionError && !sendError) return;
  ...
  useEffect(() => { if (typeof window !== "undefined") window.localStorage.setItem(PIN_STORAGE_KEY, JSON.stringify(pinnedProjectIds)); }, [pinnedProjectIds]);
  useEffect(() => { if (typeof window !== "undefined") window.localStorage.setItem(THREAD_PIN_STORAGE_KEY, JSON.stringify(pinnedConversationIds)); }, [pinnedConversationIds]);

  useEffect(() => {
    if (!selectedProjectId) { setRepositories([]); setTasks([]); setSelectedTaskId(null); return; }
    listRepositories(selectedProjectId).then(setRepositories).catch(console.error);
    listTasks(selectedProjectId).then((items) => {
      setTasks(items);
  ...
  useEffect(() => {
    if (!selectedTaskId) { setAttempts([]); setSelectedAttemptId(null); return; }
    listAttempts(selectedTaskId).then((items) => {
      setAttempts(items);
  ...
  useEffect(() => {
    if (!selectedAttemptId) { setWorkerRuns([]); setArtifacts([]); setWorktree(null); setDiff(null); setSelectedArtifactId(null); return; }
    ...
    getWorktree(selectedAttemptId).then((item) => {
      setWorktree(item);
      if (item) getWorktreeDiff(item.id).then(setDiff).catch(() => setDiff(null));
      else setDiff(null);
    }).catch(() => { setWorktree(null); setDiff(null); });
  }, [selectedAttemptId]);

  const selectedConversation = conversations.find((item) => item.id === selectedConversationId) ?? (selectedProjectId ? conversations.find((item) => item.project_id === selectedProjectId) : undefined) ?? conversations.find((item) => !item.project_id) ?? conversations[0];

  useEffect(() => {
    if (!selectedConversation) { setMessages([]); setAgentRuns([]); setSelectedAgentRunId(null); return; }
    listMessages(selectedConversation.id).then(setMessages).catch(console.error);
    listAgentRuns(selectedConversation.id).then((items) => {
      setAgentRuns(items);
  ...
  useEffect(() => {
    if (!selectedAgentRunId) { setAgentRunSteps([]); setToolCalls([]); return; }
  ...
  useEffect(() => { setExpandedDiffFileIds(diffFiles.map((file) => file.id)); }, [diffFiles.map((file) => file.id).join("|")]);

  function sortProjects(items: ProjectDto[]) {
    const sorted = [...items].sort((a, b) => {
      const aPinned = pinnedProjectIds.includes(a.id);
      const bPinned = pinnedProjectIds.includes(b.id);
      if (aPinned !== bPinned) return aPinned ? -1 : 1;
      if (sidebarSortMode === "recent") return timeValue(b.updated_at) - timeValue(a.updated_at);
      if (sidebarSortMode === "time") return timeValue(b.created_at) - timeValue(a.created_at);
      return a.name.localeCompare(b.name);
    });
    return sorted;
  }
  const sortedProjects = useMemo(() => sortProjects(projects), [pinnedProjectIds, projects, sidebarSortMode]);
  ...
  const projectConversationMap = useMemo(() => {
    ...
    for (const conversation of conversations) {
      ...
      items.sort((a, b) => sidebarSortMode === "project" ? a.title.localeCompare(b.title) : timeValue(b.updated_at) - timeValue(a.updated_at));
      map.set(conversation.project_id, items);
    }
    return map;
  }, [conversations, pinnedConversationIds, sidebarSortMode]);
  const globalConversations = useMemo(() => {
    return conversations
      .filter((item) => !item.project_id && !pinnedConversationIds.includes(item.id))
      .sort((a, b) => sidebarSortMode === "project" ? a.title.localeCompare(b.title) : timeValue(b.updated_at) - timeValue(a.updated_at))
      .slice(0, 12);
  }, [conversations, pinnedConversationIds, sidebarSortMode]);
  const visibleApprovals = useMemo(() => approvals.filter((item) => !selectedProjectId || item.project_id === selectedProjectId || item.project_id === null), [approvals, selectedProjectId]);
  ...
  ].sort((a, b) => a.time - b.time), [agentRunSteps, messages, toolCalls]);
  ...
  function toggleDiffFile(fileId: string) { setExpandedDiffFileIds((current) => current.includes(fileId) ? current.filter((id) => id !== fileId) : [...current, fileId]); }
  ...
  function moveSidebarTop() { document.querySelector(".codex-sidebar")?.scrollTo({ top: 0, behavior: "smooth" }); }
  function sidebarSortItems() {
    return [
      { label: "프로젝트 별", onSelect: () => setSidebarSortMode("project") },
      { label: "최근 프로젝트", onSelect: () => setSidebarSortMode("recent") },
      { label: "시간순 목록", onSelect: () => setSidebarSortMode("time") },
      { label: "위로 이동", onSelect: moveSidebarTop },
    ];
  }

  async function openArtifactPanel(artifact: ArtifactRefDto) {
    setSelectedArtifactId(artifact.id); setArtifactText(null); setArtifactError(null); setArtifactLoading(true); setIsInspectorOpen(true); setActiveInspectorMode("overview");
    try { const result = await readArtifact(artifact.id); setArtifactText(result.text); }
    catch (error) { setArtifactError(error instanceof Error ? error.message : String(error)); }
    finally { setArtifactLoading(false); }
  }
  function handleComposerFiles(files: FileList | null) { if (files?.length) setPendingAttachmentFiles((current) => [...current, ...Array.from(files)]); }
  function removePendingAttachment(index: number) { setPendingAttachmentFiles((current) => current.filter((_, itemIndex) => itemIndex !== index)); }
  function handleComposerPermissionChange(nextMode: string) { setComposerPermissionMode(nextMode); showActionMessage("권한 설정 저장은 다음 Grant API PR에서 연결합니다. 지금은 이 요청의 표시값으로만 반영됩니다."); }
  function handleVoiceInputPlaceholder() { showActionMessage("음성 입력은 Whisper 연결 PR에서 활성화합니다."); }

  async function handleGeneralChat() {
    setActionError(null);
    try { const conversation = await createConversation({ title: "새 채팅" }); await refreshConversations(); setSelectedConversationId(conversation.id); }
    catch (error) { setActionError(error instanceof Error ? error.message : String(error)); }
  }
  async function handleCreateProject(name: string) {
    setActionError(null);
    try { const project = await createProject({ name }); await refreshProjects(); setSelectedProjectId(project.id); }
    catch (error) { setActionError(error instanceof Error ? error.message : String(error)); }
  }
  async function handleProjectChat(project: ProjectDto) {
    setActionError(null);
    try { const conversation = await createConversation({ project_id: project.id, title: `${project.name} 채팅` }); await refreshConversations(); setSelectedProjectId(project.id); setSelectedConversationId(conversation.id); }
    catch (error) { setActionError(error instanceof Error ? error.message : String(error)); }
  }
  async function handleRenameConversation(conversation: ConversationDto, nextTitle: string) {
    setActionError(null);
    try { await conversationRequest<ConversationDto>(`/conversations/${conversation.id}`, { method: "PATCH", body: JSON.stringify({ title: nextTitle }) }); await refreshConversations(); }
    catch (error) { setActionError(error instanceof Error ? error.message : String(error)); }
  }
  async function handleRemoveConversation(conversation: ConversationDto) {
    setActionError(null);
    try { await conversationRequest<ConversationDto>(`/conversations/${conversation.id}`, { method: "DELETE" }); setPinnedConversationIds((current) => current.filter((id) => id !== conversation.id)); await refreshConversations(); }
    catch (error) { setActionError(error instanceof Error ? error.message : String(error)); }
  }
  function togglePinnedConversation(conversation: ConversationDto) { setPinnedConversationIds((current) => current.includes(conversation.id) ? current.filter((id) => id !== conversation.id) : [conversation.id, ...current]); }
  async function handleRenameProject(project: ProjectDto, nextName: string) {
    setActionError(null);
    try { await updateProject(project.id, { name: nextName }); await refreshProjects(); }
    catch (error) { setActionError(error instanceof Error ? error.message : String(error)); }
  }
  async function handleRemoveProject(project: ProjectDto) {
    setActionError(null);
    try { await removeProject(project.id); await refreshProjects(); }
    catch (error) { setActionError(error instanceof Error ? error.message : String(error)); }
  }
  async function handleOpenProject(project: ProjectDto) {
    setActionError(null);
    try { await openProjectInFileManager(project.id); }
    catch (error) { setActionError(error instanceof Error ? error.message : String(error)); }
  }
  function togglePinnedProject(project: ProjectDto) { setPinnedProjectIds((current) => current.includes(project.id) ? current.filter((id) => id !== project.id) : [project.id, ...current]); }
  ...
  async function handleSendMessage() {
    ...
    setIsSendingMessage(true); setSendError(null);
    try {
  ...
  function renderConversationRow(conversation: ConversationDto, variant: "sidebar" | "project" | "pinned") {
    ...
    return <li key={conversation.id} className="conversation-row-shell"><button type="button" className={conversation.id === selectedConversation?.id ? `${rowClass} active` : rowClass} onClick={() => setSelectedConversationId(conversation.id)}><span>{isPinned ? "★ " : ""}{conversation.title}</span></button><ActionMenu ariaLabel="채팅 더보기" items={[{ label: isPinned ? "채팅 고정 해제" : "채팅 고정", onSelect: () => togglePinnedConversation(conversation) }, { label: "채팅 이름 변경", onSelect: () => setRenameConversation(conversation) }, { label: "제거하기", destructive: true, onSelect: () => setRemoveConversationTarget(conversation) }]} /></li>;
  }
  function renderArtifactPanel() {
    if (!selectedArtifact) return null;
    return <section className="artifact-inspector-panel"><header className="artifact-inspector-header"><button type="button" className="secondary compact-button" onClick={() => setSelectedArtifactId(null)}>← Overview</button><div><h3>{artifactName(selectedArtifact)}</h3><p>{selectedArtifact.kind} · {formatBytes(selectedArtifact.size_bytes)} · {selectedArtifact.content_type}</p></div></header><div className="artifact-meta-grid"><span>ID</span><code>{selectedArtifact.id}</code><span>Created</span><small>{formatDate(selectedArtifact.created_at)}</small><span>Checksum</span><code>{shortId(selectedArtifact.checksum)}</code></div>{artifactLoading && <p className="empty-state">Artifact를 불러오는 중...</p>}{artifactError && <p className="error-inline">Artifact를 읽지 못했습니다: {artifactError}</p>}{artifactText !== null && <pre className="artifact-text-view">{artifactText}</pre>}</section>;
  }

  return <div className={isInspectorOpen ? "codex-layout" : "codex-layout inspector-collapsed"}>
    <aside className="codex-sidebar">
      <nav className="app-rail" aria-label="앱 탐색"><button type="button" className="rail-button" onClick={() => void handleGeneralChat()}>✎ <span>새 채팅</span></button><button type="button" className="rail-button" onClick={() => showActionMessage("검색은 다음 UI foundation PR에서 Command Palette로 연결합니다.")}>⌕ <span>검색</span></button></nav>
      {pinnedConversations.length > 0 && <section className="side-group pinned-group"><div className="side-title">고정된 스레드</div><ul className="sidebar-list">{pinnedConversations.map((conversation) => renderConversationRow(conversation, "pinned"))}</ul></section>}
      <section className="side-group"><div className="side-title-row"><div className="side-title">프로젝트</div><div className="side-section-actions"><button type="button" className="side-section-button" title="프로젝트 생성" onClick={() => setCreateProjectDialogOpen(true)}>＋</button><ActionMenu ariaLabel="프로젝트 사이드바 정리" items={sidebarSortItems()} /></div></div><ul className="project-list">{sortedProjects.map((project) => { const light = getProjectLight(project); const isPinned = pinnedProjectIds.includes(project.id); const projectChats = projectConversationMap.get(project.id) ?? []; return <li key={project.id} className="project-stack"><div className="project-row-shell"><button type="button" className={project.id === selectedProjectId ? "project-row active" : "project-row"} onClick={() => setSelectedProjectId(project.id)}><span className={`project-light ${light}`} title={projectLightLabel(light)} /><span className="project-name">{isPinned ? "★ " : ""}{project.name}</span></button><button type="button" className="project-icon-button" title="프로젝트 새 채팅" onClick={() => void handleProjectChat(project)}>＋</button><ActionMenu ariaLabel="프로젝트 더보기" items={[{ label: isPinned ? "프로젝트 고정 해제" : "프로젝트 고정", onSelect: () => togglePinnedProject(project) }, { label: "탐색기에서 열기", onSelect: () => void handleOpenProject(project) }, { label: "프로젝트 이름 변경", onSelect: () => setRenameProject(project) }, { label: "제거하기", destructive: true, onSelect: () => setRemoveProjectTarget(project) }]} /></div>{project.id === selectedProjectId && projectChats.length > 0 && <ul className="project-chat-list">{projectChats.map((conversation) => renderConversationRow(conversation, "project"))}</ul>}</li>; })}</ul>{projects.length === 0 && <p className="empty-state">프로젝트가 없습니다.</p>}</section>
      <section className="side-group"><div className="side-title-row"><div className="side-title">채팅</div><div className="side-section-actions"><button type="button" className="side-section-button" title="채팅 생성" onClick={() => void handleGeneralChat()}>✎</button><ActionMenu ariaLabel="채팅 사이드바 정리" items={sidebarSortItems()} /></div></div><ul className="sidebar-list">{globalConversations.map((conversation) => renderConversationRow(conversation, "sidebar"))}</ul>{!isLoadingConversations && globalConversations.length === 0 && <p className="empty-state">채팅이 없습니다.</p>}</section>
    ...
    {isInspectorOpen && <aside className="codex-review antigravity-review-shell"><header className="review-topbar antigravity-review-topbar"><button type="button" className={activeInspectorMode === "overview" ? "tab-button active" : "tab-button"} onClick={() => setActiveInspectorMode("overview")}>Overview</button><button type="button" className={activeInspectorMode === "review" ? "tab-button active" : "tab-button"} onClick={() => setActiveInspectorMode("review")}>Review</button><button type="button" className="panel-toggle" onClick={() => setIsInspectorOpen(false)}>숨기기</button></header>{selectedArtifact ? renderArtifactPanel() : activeInspectorMode === "overview" ? <section className="review-panel overview-inspector-panel"><header className="review-changes-titlebar"><h3>Overview</h3><span className={statusClass(selectedAttempt?.status)}>{statusLabel(selectedAttempt?.status)}</span></header><div className="overview-grid"><article className="status-card"><span>Task</span>{selectedTask ? <><strong>{selectedTask.title}</strong><span className={statusClass(selectedTask.status)}>{statusLabel(selectedTask.status)}</span><p>{selectedTask.instructions}</p></> : <p className="empty-state">선택된 Task가 없습니다.</p>}</article><article className="status-card"><span>Attempt</span>{selectedAttempt ? <><strong>Attempt #{selectedAttempt.attempt_number}</strong><span className={statusClass(selectedAttempt.status)}>{statusLabel(selectedAttempt.status)}</span>{selectedAttempt.result_summary && <p>{selectedAttempt.result_summary}</p>}<code>{selectedAttempt.id}</code></> : <p className="empty-state">선택된 Attempt가 없습니다.</p>}</article>{latestOwnerRun && <article className="status-card"><span>Owner 작업</span><strong>{statusLabel(latestOwnerRun.status)}</strong><p>{latestOwnerRun.purpose}</p></article>}{latestWorkerRun && <article className="status-card"><span>WorkerRun</span><strong>{shortId(latestWorkerRun.id)}</strong><span className={statusClass(latestWorkerRun.status)}>{statusLabel(latestWorkerRun.status)}</span><small>{latestWorkerRun.adapter_kind}</small>{latestWorkerRun.error_message && <p className="error-inline">{latestWorkerRun.error_message}</p>}</article>}{worktree && <article className="status-card"><span>Worktree</span><strong>{statusLabel(worktree.status)}</strong><code>{worktree.branch_name}</code><small>{worktree.worktree_path}</small></article>}{selectedRepository && <article className="status-card"><span>Repository</span><strong>{selectedRepository.repository_name}</strong><small>{selectedRepository.repository_path}</small></article>}<article className="status-card"><span>Artifacts</span>{artifacts.length > 0 ? <div className="artifact-mini-list">{artifacts.map((artifact) => <button type="button" key={artifact.id} className="artifact-mini-row" onClick={() => void openArtifactPanel(artifact)}><strong>{artifact.kind}</strong><small>{artifactName(artifact)} · {formatBytes(artifact.size_bytes)}</small></button>)}</div> : <p className="empty-state">생성된 artifact가 없습니다.</p>}</article>{visibleApprovals.length > 0 && <article className="status-card"><span>Approval Requests</span>{visibleApprovals.map((approval) => <p key={approval.id}>{approval.title} · {statusLabel(approval.status)}</p>)}</article>}{toolCalls.length > 0 && <article className="status-card"><span>Raw Tool Calls</span><pre>{JSON.stringify(toolCalls, null, 2)}</pre></article>}</div></section> : <section className="review-panel review-changes-panel"><header className="review-changes-titlebar"><div><h3>Review Changes</h3><p>{selectedTask?.title ?? "선택된 Task 없음"}</p></div><div className="review-actions"><button type="button" className="review-icon-button" title="전체 펼치기" onClick={expandAllDiffFiles}>▦</button><button type="button" className="review-icon-button" title="전체 접기" onClick={collapseAllDiffFiles}>⌃</button><button type="button" className="review-icon-button" title="검색은 다음 PR에서 연결">⌕</button></div></header><div className="review-change-summary"><span>{diffFiles.length} files changed</span><strong className="diff-addition">+{totalAdditions}</strong><strong className="diff-deletion">-{totalDeletions}</strong>{diff?.truncated && <span className="status-pill danger">truncated</span>}</div>{diffFiles.length > 0 ? <div className="changed-file-list">{diffFiles.map((file) => { const expanded = expandedDiffFileIds.includes(file.id); return <article key={file.id} className="changed-file-card"><button type="button" className="changed-file-header" onClick={() => toggleDiffFile(file.id)}><span className="file-icon">◆</span><strong>{file.path}</strong><span className="diff-addition">+{file.additions}</span><span className="diff-deletion">-{file.deletions}</span><span>{expanded ? "⌄" : "›"}</span></button>{expanded && <div className="diff-line-table">{file.lines.map((line, index) => <div key={`${file.id}-${index}`} className={`diff-line ${line.kind}`}><span className="diff-line-no">{line.oldLine ?? ""}</span><span className="diff-line-no">{line.newLine ?? ""}</span><code>{line.text}</code></div>)}</div>}</article>; })}</div> : <div className="empty-review"><h2>표시할 코드 변경이 없습니다</h2><p>Worker가 result commit을 만들거나 worktree diff가 생기면 여기에 파일별 변경 내용이 표시됩니다.</p></div>}</section>}</aside>}

    <TextInputDialog open={createProjectDialogOpen} title="프로젝트 생성" label="프로젝트 이름" initialValue="" onOpenChange={setCreateProjectDialogOpen} onSubmit={(value) => handleCreateProject(value)} />
