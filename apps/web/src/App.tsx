...
function WorkPanel({ projectId, repositories }: { projectId: string; repositories: ProjectRepositoryDto[] }) {
  ...
  async function doExternalCliDryRun(a:TaskAttemptDto){try{if(!selectedWorker)return;const v=await externalCliDryRun(a.id,selectedWorker);setWorkerRuns(x=>({...x,[a.id]:{id:v.worker_run_id,local_user_id:"",project_id:"",repository_id:null,task_id:"",task_attempt_id:a.id,worker_id:selectedWorker,adapter_kind:"external_cli_dry_run",status:v.status,started_at:null,completed_at:null,failed_at:null,cancelled_at:null,summary:null,error_code:null,error_message:null,updated_at:""}}));setArtifacts(await listArtifacts(a.id));listAttempts(selectedTask).then(setAttempts)}catch(x){setError(x instanceof Error?x.message:"External CLI dry-run failed")}}
  ...
  async function doRunAntigravity(a:TaskAttemptDto){try{if(!selectedWorker)return;const v=await runAntigravityCliExperimental(a.id,selectedWorker);setWorkerRuns(x=>({...x,[a.id]:{id:v.worker_run_id,local_user_id:"",project_id:"",repository_id:null,task_id:"",task_attempt_id:a.id,worker_id:selectedWorker,adapter_kind:"antigravity_cli",status:v.status,started_at:null,completed_at:null,failed_at:null,cancelled_at:null,summary:null,error_code:null,error_message:null,updated_at:""}}));setArtifacts(await listArtifacts(a.id));listAttempts(selectedTask).then(setAttempts)}catch(x){setError(x instanceof Error?x.message:"Antigravity CLI failed")}}
